    ...
    - generic [ref=e65]:
      ...
      - insertion [ref=e87]:
        - generic [ref=e90]:
          ...
          - link "Automation tutorials subscription" [ref=e93] [cursor=pointer]
          - link "E-commerce practice platform" [ref=e98] [cursor=pointer]
          - link "Software testing resources" [ref=e103] [cursor=pointer]
          - link "Quality assurance software" [ref=e108] [cursor=pointer]
          - link "Development Tools" [ref=e113] [cursor=pointer]
          - link "Women's Clothing" [ref=e118] [cursor=pointer]
...
> 12 |       await expect(loginPage.loginHeader).toBeVisible();
     |                                           ^ Error: toBeVisible can be only used with Locator object, was called with undefined
  13 | 
  14 |       // Submit login form and intercept network response
  15 |       const response = await loginPage.login(record.email, String(record.password));
  16 |       
  17 |       // API verification: Check if login form redirection context is healthy
  18 |       expect(response.status()).toBe(200);
  19 | 
  20 |       // UI verification
  21 |       await expect(loginPage.page.locator(`text=Logged in as ${record.name}`)).toBeVisible();
  22 | 
  23 |       // Perform Logout
  24 |       await loginPage.logoutLink.click();
  25 |       await expect(loginPage.loginHeader).toBeVisible();
  26 |     });
  27 |   }
  28 | 
  29 |   // 2. Negative Test: Invalid Credentials
  30 |   test('Should display error message with incorrect password', async ({ loginPage }) => {
  31 |     await loginPage.navigate();
  32 |     
  33 |     await loginPage.login('wrong_user@test.com', 'FakePassword123');
  34 | 
  35 |     // UI assertion verifying the specific validation error block appears
  36 |     await expect(loginPage.errorMessage).toBeVisible();
  37 |   });
  38 | 
  39 |   // 3. Multi-Page Interaction Test (Login -> Navigation)
  40 |   test('Should navigate between login page and home via navigation menu', async ({ loginPage, signupPage }) => {
  41 |     await loginPage.navigate();
  42 |     
  43 |     // Switch contexts using signup components inside the login test file safely
  44 |     await signupPage.navigate(); 
  45 |     await expect(loginPage.page).toHaveTitle(/Automation Exercise/);
  46 |   });
  47 | });
```